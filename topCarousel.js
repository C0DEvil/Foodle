...
const carouselData =
  largeDataObject.data.cards[0].card.card.imageGridCards.info;
...
const TopCarousel = () => {
  ...
  return loading ? (
    <div id="topCarouselLoader">
      <PacmanLoader color="#F9BF45" className="loaderSpinner" />
      <p
        style={{
      ...
      >
        Hungry? We're on it! Just a moment, your cravings are getting prepared
      </p>
    </div>
  ) : (
    <div id="carouselCont">
      ...
      <div id="carousel" ref={refOne} scrollLeft={scroller1}>
        {carouselData.map((item) => {
          return (
            <img
              src={TOP_CAROUSEL_IMG + item.imageId}
              key={item.id}
